...
if( !$feusers->IsValidEmailAddress( $email, -1, FALSE ) )
  {
    $params['error'] = 1;
    $params['message'] = $this->Lang('error_invalidemailaddress');
    return $this->myRedirect( $id, 'default', $returnid, $params );
  }
...
if( $this->GetPreference('allowpaidregistration') )
  {
	...
	if( $cart )
	  {	    
		...
		if( $pkg['subscr_num'] != 1 )
		  {
		    $this->Audit('',$this->GetName(),'Invalid Subscription package (more than one yer)');
		    // error.
		  }
...
if( $this->GetPreference('require_email_confirmation',1) )
...
else if ($docreatefeu)
  {
    ...
    $result = $this->_CreateFrontendUser( $tmpuid, $params['group_id'],
					  $username, $password, $expires ); // do not do md5 stuff.
	...
	if( $result[1] != $feu->LoggedInId() )
	  {
	    ...
	    if( is_array($res) && $res[0] == FALSE )
	      {
		$this->Audit('',$this->GetName(),'Problem with auto-login: '.$res[1]);
	      }
	  }
      }
