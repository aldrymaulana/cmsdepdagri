...
function fileQueueError(file, errorCode, message) {
	try {
		...
		addImage("../modules/Showtime/images/" + imageName);
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesQueued > 0) {
			...
			//this.startResizedUpload(this.getFile(0).ID, 1800, 1800, SWFUpload.RESIZE_ENCODING.JPEG, 100, false);
			this.startUpload();
		}
	} catch (ex) {
		this.debug(ex);
...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData.substring(0, 7) === "FILEID:") {
			//addImage("../modules/Showtime/function.thumbnail.php?id=" + serverData.substring(7));
			addImage(serverData.substring(7));
			progress.setStatus("Upload Complete.");
			progress.toggleCancel(false);
		} else {
			addImage("../modules/Showtime/images/error.gif");
