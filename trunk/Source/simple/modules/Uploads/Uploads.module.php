...
#BEGIN_LICENSE
#-------------------------------------------------------------------------
# Module: Uploads (c) 2011 by Robert Campbell 
...
class Uploads extends CGExtensions
{
  ...
  private
	...
	public static function getFileName( $file_id )
	{
		try
			{
				$data = uploads_utils::get_upload_details($file_id);
...
EOT;
  ...
  public function GetFriendlyName ()
  {
    ...
    parent::SetParameters();
		...
		$this->RegisterRoute('/[Uu]ploads\/file\/(?P<returnid>[0-9]+)\/(?P<upload_id>[0-9]+)\/(?P<junk>.*?)$/',
												 array('action'=>'detail'));
    ...
    $this->SetParameterType(CLEAN_REGEXP.'/field_.*/',CLEAN_STRING);
		...
		uploads_smarty::init(); // initialize smarty plugins
	}
  ...
  {
		// deprecated... this stuff should use the uploads_uploadassistant class.
		Uploads::load_common();
  ...
  {
		if( !$size ) $size = (int)$this->GetPreference('autothumbnail_size');
